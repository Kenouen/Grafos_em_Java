...
public abstract class Grafo
{
    ...
    public boolean verticeValido(Vertice vertice){
        return !vertice.getNome().equals("") && !vertice.getNome().equals(" ") && vertice.getNome().length() > 0;
    }
    ...
    public String toString() {
        ...
        for (Vertice vertex : Vertices) {
            vertices.append(vertex.toString()).append("\n");
        }

        for (Aresta aresta : Arestas) {
            arestas.append(aresta).append("\n");
        }
        vertices.append(arestas);
